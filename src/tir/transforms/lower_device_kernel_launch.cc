...
class DeviceKernelMutator : public StmtExprMutator {
 ...
 private:
  PrimExpr VisitExpr_(const CallNode* op) override {
    ...
    // std::cout << "caller target kind name: " << caller_target.get()->kind->name << "\n";
    // std::cout << "callee target kind name: " << callee_target.get()->kind->name << "\n";
    // std::cout << "caller target device type: " << caller_target->GetTargetDeviceType() << "\n";
    // std::cout << "callee target device type: " << callee_target->GetTargetDeviceType() << "\n";
    // std::cout << "caller & callee: " << caller_target->str() << ", " << callee_target->str() << "\n";
   
    // always launch kernel for c target even if the device is same, @hxf
    if (callee_target.get()->kind->name != "c") {
